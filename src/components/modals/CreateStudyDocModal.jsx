...
import { X, UploadCloud, BookOpen, RotateCcw, Download, FileText } from 'lucide-react';
...
const CreateStudyDocModal = ({ isOpen, onClose, projectId, documentId, onSuccess }) => {
  ...
  const [title, setTitle] = useState(`Tài liệu phòng thi ${new Date().toLocaleDateString('vi-VN')}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const sources = [
        ...uploadedSources.map(s => ({ type: 'text', value: s.text, name: s.name })),
        ...librarySources.map(s => ({ type: s.type, value: String(s.value), name: s.name })),
      ];
      if (activeDocId) sources.push({ type: 'document', value: String(activeDocId), name: null });
      const payload = {
        title,
        description,
        objective: target,
      ...
      const res = await fetch('http://127.0.0.1:8000/api/examdocs/generate', {
        method: 'POST',
      ...
      <div style={{
        ...
        <div className="animate-fade-in" style={{
          ...
          <div style={{ padding: '20px 24px', backgroundColor: 'var(--bg-tertiary)', borderTop: '1px solid var(--border-light)', display: 'flex', justifyContent: 'center', gap: '12px', borderRadius: '0 0 24px 24px', flexShrink: 0 }}>
            ...
            <button onClick={handleDownloadMd} style={{ padding: '12px 28px', backgroundColor: 'var(--bg-tertiary)', border: '1px solid #8A334C', borderRadius: '24px', fontWeight: 600, color: '#8A334C', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      <div className="animate-fade-in" style={{
        ...
        <div style={{ padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-light)', position: 'sticky', top: 0, backgroundColor: '#FCFAF8', zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <h2 style={{ fontSize: '1.25rem', fontWeight: 900, color: 'var(--text-navy)' }}>Cấu hình Tài liệu phòng thi</h2>
          </div>
        ...
        <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ border: '1px dashed var(--border-medium)', borderRadius: '16px', padding: '16px', backgroundColor: '#FDF8F5', marginTop: '8px' }}>
            ...
            <div style={{ marginTop: '16px', padding: '12px', backgroundColor: 'var(--bg-tertiary)', border: '1px solid var(--border-light)', borderRadius: '12px', fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)' }}>
              Tổng dung lượng nguồn: {(uploadedSources.reduce((n, s) => n + s.text.length, 0) / 1024).toFixed(1)} KB / 20 MB · {librarySources.length} tài liệu thư viện
            </div>
          </div>
